...
func main() {
	...
	handler := []gin.HandlerFunc{worker.ConcurrencyLimiter(), worker.OpenAIProtocolRegisterHandler}
	r.POST("/api/v1/process", handler...)
	// 兼容 VPS 直连隧道域名（不经过 CF Worker 路由重写时路径为 /openai/process）
	r.POST("/openai/process", handler...)
